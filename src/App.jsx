        ...
        <main className="grid grid-cols-1 md:grid-cols-2 gap-8 lg:gap-12 max-w-5xl mx-auto">
          {pageConfig.links.map((link) => (
            <a 
              ...
              className="group relative overflow-hidden rounded-2xl bg-stone-100 aspect-[4/3] flex flex-col justify-end shadow-sm hover:shadow-xl transition-all duration-500 hover:-translate-y-1"
            >
              ...
              <div className="relative p-8 z-10">
                <div className="transform transition-transform duration-500 group-hover:-translate-y-2">
                  <span className="inline-block px-3 py-1 bg-white/20 backdrop-blur-md border border-white/30 rounded-full text-xs text-white/90 uppercase tracking-wider mb-3">
                    Inspiração
                  </span>
                  <h2 className="text-3xl font-medium text-white mb-2">
                    {link.title}
                  </h2>
                  <p className="text-stone-200 font-light text-sm md:text-base opacity-0 group-hover:opacity-100 transition-opacity duration-500 delay-100 h-0 group-hover:h-auto group-hover:mt-2">
                    {link.description}
                  </p>
                </div>
                
                <div className="absolute bottom-8 right-8 w-10 h-10 bg-white rounded-full flex items-center justify-center transform translate-y-8 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-500 shadow-lg delay-75">
